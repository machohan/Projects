...
void my_exit_group(int status)
{
	...
	//call delete pid for syscall from function above.
	spin_lock(&pidlist_lock);
	del_pid(current->pid);
	spin_unlock(&pidlist_lock);
	
	//call to original exit group.
...
asmlinkage long interceptor(struct pt_regs reg) {
	...
	//log intercepted syscall info to kernel log
...
asmlinkage long my_syscall(int cmd, int syscall, int pid) {
	...
	valid = pid_task(find_vpid(pid), PIDTYPE_PID);
	
	//schedular has pid=0 but in this assignment it mean monitor all pids.
	if(pid < 0 ||  (valid == NULL && pid != 0 ) ){
		return -EINVAL;
	}	
	
	//checking for root user for first two commands.
	if(cmd == REQUEST_SYSCALL_INTERCEPT || cmd == REQUEST_SYSCALL_RELEASE){
		...
		if(cmd == REQUEST_SYSCALL_INTERCEPT){
			...
			// perform necessary changes in mytable struct for interception of syscall.
			spin_lock(&pidlist_lock);
			...
			//Set syscall table to writeable.
			spin_lock(&calltable_lock);
			set_addr_rw((unsigned long)sys_call_table);
			sys_call_table[syscall] = &interceptor;
			
			//Set syscall table to readonly.
		...
		if(cmd == REQUEST_SYSCALL_RELEASE){
			...
			//is syscall already intercepted? if so -EINVAL
			if(table[syscall].intercepted == 0){
				return -EINVAL;
			}
			
			//destroy pid list for de-intercepted syscall.
			spin_lock(&pidlist_lock);
			destroy_list(syscall);
			spin_unlock(&pidlist_lock);
			
			//change intercepted syscall to zero.
			spin_lock(&pidlist_lock);
		...
		if(cmd == REQUEST_START_MONITORING){
			...
			//Is syscall for which we want to monitor pid already intercepted?
			if(table[syscall].intercepted == 0){
				return -EINVAL;
			}
			
			//check if pid already being monitored
			if(pid != 0){
			...
			//don't add pid to the list if all pids are monitored: do nothing
		...
		if(cmd == REQUEST_STOP_MONITORING){
			...
			//already monitored? return -EINVAL
			spin_lock(&pidlist_lock);
			
			//excluding pid 0, check if pid is being monitored.
			...
			//if pid is not 0, delete pid from pid_list of syscall.
			spin_lock(&pidlist_lock);
			
			if( pid!=0 && del_pid_sysc(pid, syscall) == -EINVAL){
				spin_unlock(&pidlist_lock);
				return -EINVAL;
			}
			...
			//update monitored flag accordingly
	...
	else {//for any other unacceptable cmd.
		return -EINVAL;
	}
...
void* sys_call_table_copy[NR_syscalls+1];
static int init_function(void) {
	...
	//Set syscall table to writeable atomically.
	spin_lock(&calltable_lock);
	set_addr_rw((unsigned long)sys_call_table);
	
	//hijacking my_custom_syscall
	orig_custom_syscall = sys_call_table[0];
	sys_call_table[0] = &my_syscall;
	
	//hijacking orig_exit_group syscall
	...
	//initializing my_list for each syscall in mytable.
	for (i=0; i<=NR_syscalls; i++){
		INIT_LIST_HEAD (&table[i].my_list);
		table[i].intercepted = 0; //initializing all syscalls are not  intercepted.
		table[i].monitored = 0; //initializing all syscalls are not  monitored.
	}
	spin_unlock(&calltable_lock);
...
{        
	...
	//Set syscall table to write only.
	spin_lock(&calltable_lock);
	set_addr_rw((unsigned long)sys_call_table);
	
	//Restoring syscall table to default.
	for (i=0; i<=NR_syscalls; i++){
